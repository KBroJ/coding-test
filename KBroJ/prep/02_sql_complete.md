...
| 파트 | 우선순위 | 추천 문제 |
|------|---------|----------|
| SELECT | ★ | 여러 기준으로 정렬하기, 조건에 맞는 도서 리스트 출력하기 |
| GROUP BY | ★★★ | 자동차 종류 별 특정 옵션이 포함된 자동차 수 구하기, 입양 시각 구하기(1)(2) |
| IS NULL | ★★ | 이름이 없는 동물의 아이디, 경기도에 위치한 식품창고 목록 출력하기 |
| JOIN | ★★★ | 없어진 기록 찾기, 있었는데요 없었습니다, 오랜 기간 보호한 동물(1)(2) |
| String/Date | ★★ | 자동차 대여 기록에서 대여중/대여 가능 여부 구분하기, 조건에 맞는 사용자와 총 거래금액 조회하기 |
| Window Function | ★ | 그룹별 조건에 맞는 식당 목록 출력하기, 취소되지 않은 진료 예약 조회하기 |

### 자주 나오는 패턴 Top 5 (기출 분석)

1. **DATE_FORMAT(컬럼, '%Y-%m-%d')** - 날짜 관련 문제에 거의 필수 등장
2. **LEFT JOIN + IS NULL** - "A에는 있는데 B에는 없는" 데이터 찾기
3. **GROUP BY + HAVING COUNT(*)** - 특정 조건을 만족하는 그룹 필터링
4. **서브쿼리 in WHERE** - `WHERE 컬럼 IN (SELECT ...)`
5. **RANK() / DENSE_RANK() OVER (PARTITION BY ... ORDER BY ...)** - 그룹 내 순위
